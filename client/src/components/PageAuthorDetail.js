...
class PageAuthorDetails extends Component {
  ...
  componentDidMount() {
    ...
    getData(URL.author + id).then((data) => this.setState(data))
  }
  render() {
    const { author, author_books } = this.state
    return (
      <div>
        <WindowTitle title="Author Details" />
        ...
        <div>
          ...
          <dl>
            ...
            {isEmpty(author_books) && <p>No book found for this author.</p>}
          </dl>
        </div>
      </div>
    )
